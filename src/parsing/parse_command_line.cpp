...
ParseResult ParseCommandLineArguments(int argc, char* argv[]) {
  ...
  description.add_options()("help", "produce help message")(
      ...
      "backup_directories", program_options::value<std::vector<std::string>>(),
      "directories to do backup from and to");
  ...
  program_options::store(program_options::command_line_parser(argc, argv)
                             .options(description)
                             .positional(positional_descriptions)
                             .run(),
                         variables_map);
  ...
  if (variables_map.contains("backup_directories")) {
    const auto backup_directories =
        variables_map["backup_directories"].as<std::vector<std::string>>();
    if (backup_directories.size() != 2) {
      throw std::length_error(std::format(
          "expected exactly 2 arguments for backup directories, got {}",
          backup_directories.size()));
    }
